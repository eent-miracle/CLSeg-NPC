...
def parse_args():    
    ...
    parser.add_argument('-point_folder', type=str, default='./datasets/', help='the folder putting masks')
    ...
    parser.add_argument('-region_type', type=str, default='fixed_pair', help='condition type:ave,rand,rand_map')
    ...
    parser.add_argument('-with_boxes', type=lambda x: bool(strtobool(str(x))), default=False, help='with boxes')
    parser.add_argument('-with_points', type=lambda x: bool(strtobool(str(x))), default=False, help='with points')
    parser.add_argument('-point_drop_prob', type=float, default=0.0, help='random dropout prob for point prompts (train only)')
    parser.add_argument('-box_drop_prob', type=float, default=0.0, help='random dropout prob for box prompts (train only)')
    parser.add_argument('-point_drop_high', type=float, default=None, help='high-stage point dropout prob (override point_drop_prob)')
    parser.add_argument('-point_drop_low', type=float, default=None, help='low-stage point dropout prob (override point_drop_prob)')
    parser.add_argument('-box_drop_high', type=float, default=None, help='high-stage box dropout prob (override box_drop_prob)')
    parser.add_argument('-box_drop_low', type=float, default=None, help='low-stage box dropout prob (override box_drop_prob)')
    parser.add_argument('-dropout_switch_ratio', type=float, default=0.5, help='epoch ratio to switch from high to low dropout')
    parser.add_argument('-keep_one_prompt', type=lambda x: bool(strtobool(str(x))), default=True, help='ensure at least one point remains after dropout')
    parser.add_argument('-iou_loss_weight', type=float, default=1.0, help='weight for IoU head supervision loss')
    # Pseudo-mask filtering thresholds for S->C
    parser.add_argument('-eta', type=float, default=0.6, help='confidence threshold u_i')
    parser.add_argument('-alpha', type=float, default=0.6, help='LCC dominance threshold')
    parser.add_argument('-beta', type=float, default=4.0, help='shape regularity threshold')
    parser.add_argument('-disable_filter', type=lambda x: bool(strtobool(str(x))), default=False, help='disable pseudo-mask filtering and keep all masks')
    parser.add_argument('-threshscore', type=float, default=0.4, help='score threshold for pseudo masks')
