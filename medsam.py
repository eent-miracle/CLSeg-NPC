...
class ImageEncoderViT(nn.Module):
    ...
    def init_head_weights(self, std: float = 2e-5):
        # Initialize all linear layers inside the head consistently
    ...
    def init_weights(self):
        if self.pretrained is not None:
            ...
            for key_init in list(state_dict.keys()):
                ...
                if 'mask_encoder.' in key_init or 'neck.' in key_init or 'mask_decoder.' in key_init or 'prompt_encoder.' in key_init:
                    state_dict.pop(key_init)
                    continue
                    
                state_dict[key] = state_dict.pop(key_init)
            # print('[debug xue] state_dict.keys():', state_dict.keys())
